...
public class ManifestFile {
    ...
    try {
      ...
      if(snpCount_ == null) {
        throw new IOException(
            "Invalid file format. Expected one of [" + HEADER_LOCI_COUNT + "," + HEADER_SNP_COUNT + "] header values.");
      }
      this.manifestFileEntries_ = new ArrayList<ManifestFileAssayEntry>(snpCount_);
      parseSnps(lnr);
      if(manifestFileEntries_.size() != snpCount_.intValue()) {
        throw new IOException(
            "Invalid file format. Manifest contains " + manifestFileEntries_.size() + " entries, but header reported " +
                snpCount_ + " snps.");
      }
    } catch(Exception e) {
  ...
  private void handleHeader(String key, String value) throws IOException {
    ...
    if(HEADER_LOCI_COUNT.equalsIgnoreCase(key) == true || HEADER_SNP_COUNT.equalsIgnoreCase(key) == true) {
      if(snpCount_ != null) {
        throw new IOException(
            "Invalid header section: only one of \"" + HEADER_LOCI_COUNT + "\" and \"" + HEADER_SNP_COUNT +
                "\" should be present.");
      }
      ...
      try {
        ...
        throw new IOException(
            "Invalid header value: the value [" + value + "] for the header \"" + key + "\" should be a number.");
      }
    }
  }
  ...
  private void parseSnps(LineNumberReader lnr) throws IOException {
    ...
    for(int i = 0; i < snpCount_; i++) {
      ...
      if(entryLine == null) {
        throw new IOException(
            "Invalid file format. Expected " + snpCount_ + " SNPs in " + SNP_START_TOKEN + " section found only " + i +
                ".");
      }
