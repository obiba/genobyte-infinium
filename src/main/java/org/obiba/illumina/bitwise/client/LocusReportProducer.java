...
public class LocusReportProducer implements ReportProducer {
  ...
  public void generateReport(CliContext context, String[] parameters, PrintStream output) {
    ...
    StatsPool<Integer, String> assayStatsPool = new StatsPool<Integer, String>(store,
        new DefaultAssayStatsRunDefinition());
    ...
    context.getOutput().println(
        "Calculating locus report for " + assayStatsPool.getRecordMask().count() + " assays on " +
            assayStatsPool.getTransposedMask().count() + " samples.");
