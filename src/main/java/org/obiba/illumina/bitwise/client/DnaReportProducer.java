...
public class DnaReportProducer implements ReportProducer {
  ...
  public void generateReport(CliContext context, String[] parameters, PrintStream output) {
    ...
    StatsPool<String, Integer> sampleStatsPool = new StatsPool<String, Integer>(store,
        new DefaultSampleStatsRunDefinition());
    ...
    context.getOutput().println(
        "Calculating DNA report for " + sampleStatsPool.getRecordMask().count() + " samples on " +
            sampleStatsPool.getTransposedMask().count() + " assays.");
