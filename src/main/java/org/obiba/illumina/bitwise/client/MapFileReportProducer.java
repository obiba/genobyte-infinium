...
public class MapFileReportProducer implements ReportProducer {
  ...
  public void generateReport(CliContext context, String[] parameters, PrintStream output) {
    ...
    FieldValueIterator<Chromosome> chromosomes = new FieldValueIterator<Chromosome>(
        store.getStore().getField("chromosome"), assays);
    FieldValueIterator<String> names = new FieldValueIterator<String>(store.getStore().getField("snpName"), assays);
    FieldValueIterator<Integer> positions = new FieldValueIterator<Integer>(store.getStore().getField("mapInfo"),
        assays);
    ...
    while(chromosomes.hasNext()) {
      ...
      sb.append(outputChromosome(chr)).append(' ').append(name).append(' ').append(position);
